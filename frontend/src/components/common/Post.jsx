...
const Post = ({ post }) => {
	...
	const { mutate: commentreply, isPending: isReplying } = useMutation({
		mutationFn: async () => {
			try {
													...
													<div className='flex items-center gap-3 justify-between'>
														...
														<button
															...
															onClick={() => {setCommentid(comment._id);
																	document.getElementById("replies_modal" + comment._id).showModal();}}>
															Reply
														</button>
														

														 
														    <dialog id={`replies_modal${comment._id}`} className='modal border-none outline-none'>
															<div className='modal-box rounded border border-gray-600'>
																<h3 className='font-bold text-lg mb-4'>REPLIES</h3>
																<div className='flex flex-col gap-3 max-h-60 overflow-auto'>
																	{comment.replies.length === 0 && (
																		<p className='text-sm text-slate-500'>
																			No replies for the comment yet🤔 
																		</p>
																	)}
																	{comment.replies.map((reply) => (
																		<div key={reply._id} className='flex gap-2 items-start'>
																			...
																			<div className='flex flex-col'>
																				...
																				<div className='text-sm mt-2'>{reply.text}</div>
																			</div>
																...
																>
																	...
																	<button className='btn btn-primary rounded-full btn-sm text-white px-4'>
																		{isReplying ? <LoadingSpinner size='md' /> : "Post"}
																	</button>
																</form>
															</div>
